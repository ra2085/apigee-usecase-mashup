...
apigeecli keyaliases create --alias saml-key --format keycertfile --certFilePath ./sa.crt --keyFilePath ./sa.key --org "$PROJECT" --env "$APIGEE_ENV" --token "$TOKEN"
apigeecli keyaliases create --alias saml-crt --format keycertfile --certFilePath ./sa.crt --org "$PROJECT" --env "$APIGEE_ENV" --token "$TOKEN"
